...
# After a BUY or SELL alert fires, we remember the direction and entry price
# so we can watch for signs the move is reversing and warn the user to exit/hold.
active_position = None       # "BUY", "SELL", or None
entry_price = None
PRICE_REVERSAL_PCT = 0.3     # price moving this % against the position triggers a warning
RSI_REVERSAL_BUY = 70        # after a BUY, RSI spiking above this = momentum flipping
RSI_REVERSAL_SELL = 30       # after a SELL, RSI dropping below this = momentum flipping
...
def check_for_reversal(price, ma5, ma10, rsi):
    ...
    Combines: MA5 crossing back against the trend, RSI swinging hard the
    other way, and price moving against entry by a threshold %.
    """
    global active_position, entry_price

    if active_position is None or entry_price is None:
        return None

    reasons = []

    # 1. MA5 crossing back against the trend
    ...
    # 2. RSI swinging hard the other way
    if rsi is not None:
    ...
    # 3. Price moving against entry by threshold %
...
async def poll_and_alert(context: ContextTypes.DEFAULT_TYPE):
    global snoozed_until, active_position, entry_price
    ...
    # Check for a reversal warning on any currently tracked position first
    ...
    if signal in ("BUY", "SELL"):
        ...
        try:
            ...
            # Start tracking this as the active position for reversal monitoring
